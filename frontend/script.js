(() => {
  ...
  function renderResult(data) {
    ...
    (data.top_factors || []).forEach((f) => {
      ...
      li.className = `factor-row factor-${f.direction === "increases" ? "up" : "down"}`;
    ...
    modelInfoEl.textContent = data.model_version
  ...
  if (batchForm) {
    batchForm.addEventListener("submit", async (e) => {
      ...
      batchResultEl.innerHTML = `<p class="batch-status">Scoring ${file.name}…</p>`;
      ...
      try {
        ...
        if (!res.ok) {
          ...
          batchResultEl.innerHTML = `<p class="batch-status batch-status-error">${detail}</p>`;
          return;
        }
        ...
        const rows = body.results
          .map((r) => {
            if (r.error) {
              return `<tr class="batch-row-error"><td>${r.row_index}</td><td colspan="2">${r.error}</td></tr>`;
            }
            return `<tr><td>${r.row_index}</td><td>${r.predicted_mental_health_score.toFixed(2)}</td><td>—</td></tr>`;
          })
          .join("");

        batchResultEl.innerHTML = `
          <p class="batch-status">${body.successful} of ${body.total_rows} rows scored successfully.</p>
          <table class="batch-table">
            <thead><tr><th>Row</th><th>Score</th><th></th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        `;
      } catch (err) {
        batchResultEl.innerHTML = `<p class="batch-status batch-status-error">Couldn't reach ${API_BASE}. Make sure the backend is running.</p>`;
      } finally {
